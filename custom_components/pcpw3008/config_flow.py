...
class PcPw3008ConfigFlow(ConfigFlow, domain=DOMAIN):
    """Pair the scale, then collect the profile it needs."""
    ...
    async def async_step_bluetooth(
        ...
        """Triggered by the manifest's Bluetooth matcher."""
    ...
    async def async_step_user(
        ...
        """Pick from scales Home Assistant has already seen advertising."""
        ...
        if not candidates:
            # The scale only advertises while awake.
    ...
    async def async_step_profile(
        ...
        if user_input is None:
            return self.async_show_form(
                step_id="profile", data_schema=PROFILE_SCHEMA
            )
